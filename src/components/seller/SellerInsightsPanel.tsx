...
export default function SellerInsightsPanel({ sellerId }: Props) {
  ...
  if (loading) {
    return <p className="text-sm text-gray-500 py-2">{t('common.loading')}</p>;
  }
  ...
  if (insights.totalSales === 0) {
    return (
      <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/80 px-4 py-6 text-center">
        ...
        <p className="text-sm font-medium text-gray-700">{t('sellerInsights.empty')}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-4">
        {tiles.map((tile) => (
          ...
          >
            ...
            <span className="text-[10px] text-gray-400 truncate">{tile.sub}</span>
          </div>
        ))}
      </div>
          ...
          {insights.monthlyBuckets.map((b) => {
            ...
            return (
              <li key={b.month} className="flex items-center gap-2 text-xs">
                <span className="w-12 shrink-0 text-gray-500">{monthLabel(b.month, locale)}</span>
                ...
                <span className="w-24 shrink-0 text-right font-semibold text-gray-700 tabular-nums">
                  {formatPrice(b.revenue)}
                </span>
                <span className="w-8 shrink-0 text-right text-gray-400 tabular-nums">{b.count}</span>
              </li>
            );
          })}
        </ul>
          ...
          {insights.topProducts.map((p) => (
            ...
            >
              <Link
                ...
                className="flex-1 truncate text-gray-800 hover:underline"
              >
                {p.name}
              </Link>
              <span className="shrink-0 text-gray-500 tabular-nums">×{p.soldCount}</span>
