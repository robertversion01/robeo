...
export default function Navbar({ searchQuery, onSearchChange }: NavbarProps) {
  ...
  const profileMenu = loggedIn ? (
    <div ref={profileMenuRef} className="relative z-[10000] shrink-0">
      <button
        type="button"
  ...
  const guestDesktopActions = loading ? (
    ...
    <>
      ...
      <LanguageSwitcher variant="light" className="hidden sm:inline-flex shrink-0" />
      <Link
    ...
    <nav className="fixed top-0 left-0 right-0 z-[9999] h-11 bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="mx-auto flex h-full max-w-6xl items-center gap-1.5 px-2 sm:gap-2 sm:px-3">
        ...
        <div className="hidden md:flex items-center gap-1 shrink-0">
          {loggedIn ? loggedInDesktopActions : guestDesktopActions}
        </div>
