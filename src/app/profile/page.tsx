      ...
      <main className="pt-16 pb-12 px-3 md:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-14 h-14 rounded-full bg-accent/10 dark:bg-accent/20 flex items-center justify-center text-accent text-xl font-bold">
              {user?.email?.charAt(0).toUpperCase() || '?'}
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold">Profilom</h1>
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-white/50 mt-1">
          ...
          <p className="text-gray-500 dark:text-white/60 text-sm mb-6 ml-0">Saját feltöltött termékeim</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-400 dark:text-white/50 text-xs">({stats.reviewCount})</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-red-50 dark:bg-red-500/20 hover:bg-red-100 dark:hover:bg-red-500/30 text-red-500 dark:text-red-400 rounded-lg text-xs transition-colors"
            >
              Összes törlése
            </button>
          </div>
          ...
          {products.length === 0 ? (
            ...
            <>
              <p className="text-gray-500 dark:text-white/50 text-sm mb-4">{products.length} termék</p>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {products.map((product) => (
                  <div 
                    key={product.id} 
                    className="group bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-lg overflow-hidden relative"
                  >
                    <Link href={`/products/${product.id}`} className="aspect-[4/5] overflow-hidden block">
                      {product.image_url ? (
                        ...
                        <div className="w-full h-full bg-gray-100 dark:bg-white/10 flex items-center justify-center text-gray-400 dark:text-white/30">
                          📷
                        </div>
                      )}
                    </Link>
                    ...
                    <div className="p-1.5 space-y-0.5">
                      <div className="text-gray-500 dark:text-white/50 text-[8px] uppercase tracking-wider">
