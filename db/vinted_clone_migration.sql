...
create policy if not exists "chat_media_upload_authenticated"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'chat-media');

create policy if not exists "chat_media_read_public"
on storage.objects
for select
to public
...
create policy if not exists "transactions_insert_checkout"
on public.transactions
for insert
to anon, authenticated
with check (true);

create policy if not exists "transactions_update_participants"
on public.transactions
for update
to authenticated
...
create policy if not exists "transactions_select_participants"
on public.transactions
for select
to authenticated
using (auth.uid() = buyer_id or auth.uid() = seller_id);
